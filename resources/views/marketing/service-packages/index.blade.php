...
@section('content')
    ...
    <div class="flex items-center">
        <h2 class="text-2xl font-bold leading-none tracking-tight text-black md:text-3xl dark:text-slate-200 ">Paket Layanan
        </h2>
                ...
                @foreach ($datas as $data)
                    <tr
                        ...
                        <td class="px-6 py-4 flex font-normal whitespace-nowrap dark:text-white">
                            <a href="{{ url('service-packages/' . $data->id . '/edit') }}"
                                class="mr-2 font-medium text-blue-600 dark:text-blue-500 hover:underline"><span>Edit</span></a>
                            <form name="deletePackages" action="{{ url('service-packages/' . $data->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
...
@include('marketing.service-packages.modal-create')
